...
// make a function here to initialise ADC1 on both required pins
void initADC1ForCurrent( void )
{
    // Enable ADC1 and the right GPIO pins
    SysCtlPeripheralEnable( SYSCTL_PERIPH_ADC1 );
    SysCtlPeripheralEnable( SYSCTL_PERIPH_GPIOE );
    SysCtlPeripheralEnable( SYSCTL_PERIPH_GPIOD );

    // Sets the GPIO pins as input and ANALOG
    // ISENC
    GPIOPinTypeADC( GPIO_PORTE_BASE, GPIO_PIN_3 );
    // ISENB
    GPIOPinTypeADC( GPIO_PORTD_BASE, GPIO_PIN_7 );

    // Coupla defines for configuring the sequencer
    #define ADC_SEQ 1

    // Configure the ADC sequence
    ADCSequenceConfigure( ADC1_BASE, ADC_SEQ, ADC_TRIGGER_PROCESSOR, 0 );

    // Configure step 1 of 4 sequence steps available
    // ISENC (PE_3, AIN0)
    ADCSequenceStepConfigure( ADC1_BASE, ADC_SEQ, 0, ADC_CTL_CH0 );
    // Configure step 2 of 4 sequence steps available
    // ISENB (PD_7, AIN4)
    ADCSequenceStepConfigure( ADC1_BASE, ADC_SEQ, 1, ADC_CTL_CH4 | ADC_CTL_IE | ADC_CTL_END );

    // Enable to ADCSequence
...
// values to hold the values of the phases over time
volatile uint32_t og_PhaseCCurrentVal = 0;
volatile uint32_t og_PhaseBCurrentVal = 0;
// The interrupt handler for the ACD1
void ADC1IntHandler( void )
{
    ...
    uint32_t pui32ADC1Values[2];
    ADCSequenceDataGet(ADC1_BASE, ADC_SEQ, pui32ADC1Values);

    // Store the values
    og_PhaseCCurrentVal = pui32ADC1Values[0];  // Value from PE3 (AIN0)
    og_PhaseBCurrentVal = pui32ADC1Values[1];  // Value from PD7 (AIN4)
}

// struct to hold the information from the 2 phases
typedef struct {
    uint32_t PhaseB;
    uint32_t PhaseC;
} ADC1Values;
// should read and update the current values everytime a timer interrupts
// executes at 150Hz
ADC1Values ReadCurrentFromACD1 ( void )
{
    // variables to get copies from the current vals
    ADC1Values Current_Vals;

    // Trigger ADC conversion
    ADCProcessorTrigger(ADC1_BASE, 1);

    // Get the latest ADC values
    Current_Vals.PhaseC = og_PhaseCCurrentVal;
    Current_Vals.PhaseB = og_PhaseBCurrentVal;
